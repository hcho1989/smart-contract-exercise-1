...
async function main() {
  ...
  const signableADaiContract = aDaiContract.connect(mywallet);
  ...
  const txresp = await signableADaiContract.approve(myContractAddr, amountToDeposit)
  await txresp.wait();

  console.log("my allowance aDAI for mycontract", await checkAllowance(aDaiContract, myAddress, myContractAddr))
  
  // call deposit function
  const txresp2 = await signableMyContract.withdraw(daiContractAddress, amountToDeposit, {gasLimit: 500000})
  await txresp2.wait();
